...
echo "Домен: $NUXT_DOMAIN, email: $NUXT_EMAIL"
...
if [ "$NUXT_DOMAIN" = "" ]; then
  echo "Не установлена переменная окружения NUXT_DOMAIN для домена" >&2
  exit 1
else
  if [ "$NUXT_EMAIL" = "" ]; then
    echo "Не установлена переменная окружения NUXT_EMAIL для email для Let's Encrypt" >&2
    exit 1
  fi
fi
...
# список доменов для Let's Encrypt из переменной окружения
domains=($NUXT_DOMAIN)
